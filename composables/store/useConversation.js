...
export const getConversations = async (conversationIds, conversations, append) => {
  ...
  const { data, refresh, error, pending } = await useApi(`conversations/get-conversations-data`,
    {
      initialCache: false,
      body: payload,
      method: "POST",
      ...
      conversations.forEach((conversation) => {
        if (conversation.id == user.id) {
          conversationsUsers.push({
            user: user.participants[0].firebase_uuid == authStore.getAuthUser.firebase_uuid ? user.participants[1] : user.participants[0],
...
export const getOrCreateConversation = async (id, type) => {
  const { data, refresh, error, pending } = await useApi(
    ...
    {
      initialCache: false,
      body: (type == 1) ? { type: type, recipient_id: id } : { type: type, conversationable_id: id },
      method: "POST",
    }
  )
  if (data.success) {
