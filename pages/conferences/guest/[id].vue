<template>
    <div class="max-w-screen-xl mx-auto py-4">
            ...
            <div class="md:col-span-2 sticky top-4 ">
                <div v-if="conference">
                    <div
                        ...
                        <ul role="list" class="space-y-3 text-sm leading-6 dark:text-gray-300 xl:mt-2">
                            <li class="flex items-center gap-x-3">
                                ...
                                <span>{{ $t('Date') }} :</span>
                                <span class="font-bold">{{ $dayjs(conference?.date).format('D/M/YYYY hh:mm A')
                                    }}</span>
                            </li>
                        ...
                        <div class="py-4">
                            <UButton block :loading="submitLoading" size="lg"
                                ...
                                v-if="!IsPassed && !conferenceGotStarted" @click="subscribeUser" label="Subscribe" />


                            <UButton block @click="joinConferenceModal = true" size="lg"
                            ...
                            <UButton v-if="!IsPassed && (!paymentSuccess || paymentSuccess != 1) && !conferenceStarted"
                    ...
                    <UAlert v-if="paymentSuccess" icon="i-heroicons-check" color="green" variant="subtle"
                        ...
                        description="Please check your email for the credentials to join the conference." />
                </div>
        ...
        <UModal v-model="isOpen" prevent-close>
            <UCard class="w-full p-8 relative z-50 overflow-hidden">
                ...
                <div class="flex flex-col items-center justify-center ">
                    <img class="w-1/3 flex dark:hidden" src="~/assets/svg/vectors/conference.svg" alt="">
                    <img class="w-1/3 hidden dark:flex" src="~/assets/svg/vectors/conference-white.svg" alt="">
                    <h2 class="text-xl md:text-3xl font-bold">{{ $t('Subscribe to conference') }}</h2>
        ...
        <UModal v-model="joinConferenceModal">
            <UCard class="w-full p-0 pt-0 relative z-50 overflow-hidden">
                <template #header>
                    <div class="flex items-center justify-between">
                        ...
                        <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark-20-solid" class="-my-1 z-50"
                            @click="joinConferenceModal = false" />
                    </div>
                </template>
                <img src="~/assets/svg/vectors/pattern-rectangle.svg" draggable="false"
                    class="w-12 absolute top-0 right-0" alt="" srcset="">
                <div class="flex flex-col items-center justify-center ">
                    <img class="w-1/3 flex dark:hidden" src="~/assets/svg/vectors/conference.svg" alt="">
                    <img class="w-1/3 hidden dark:flex" src="~/assets/svg/vectors/conference-white.svg" alt="">
                    <h2 class="text-xl md:text-3xl font-bold">{{ $t('Join to conference') }}</h2>
                    <p class="text-blueGray-900 dark:text-slate-300">{{ $t('login.please_log_in') }}</p>
                </div>
                ...
                <UForm ref="form" :schema="externalUserJoinSchema" :state="externalUserJoinState"
                    ...
                    <div class="flex flex-col justify-between space-y-2">
                        <UButton v-if="conferenceStarted && !IsPassed" block :loading="loadingSubmit" size="lg"
                            :color="$colorMode.value == 'dark' ? 'green' : 'primary'" type="submit">
                            {{ $t('Submit') }}
                        </UButton>

                        <UButton :disabled="true" v-if="!conferenceStarted && !IsPassed" block :loading="loadingSubmit"
                            size="lg" :color="$colorMode.value == 'dark' ? 'green' : 'primary'" type="submit">
                            {{ $t('Conference started') }}: {{ countdownLabel }}
                        </UButton>
                    </div>
                    <div v-if="!IsPassed">
                        <UDivider label="" class="my-2" />
                        <UButton block @click="{ joinConferenceModal = false; resendCredentialsModal = true; }"
...
<script setup>
import { showConference, externalUserJoin, guestSubscribeConference, resendExternalCredentials, resendExternalAnimatorCredentials, externalAnimatorJoin } from '~/composables/store/useConference'
...
const joinConferenceModal = ref(false)
const paymentSuccess = route.query?.payment_successfully
const conferenceGotStarted = ref(false)
...
const calculateCountdown = () => {
    ...
    if (days == 0 && hours == 0 && minutes == 0 && seconds == 0) {
        conferenceGotStarted.value = true
    }
}



watch(conferenceGotStarted, async (newValue, oldValue) => {
    if (newValue) {
...
const submitSubscribeForm = async () => {
    ...
    try {
        const result = await guestSubscribeConference(id, subscribeState)
